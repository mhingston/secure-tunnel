...
Secure Tunnel is a native Rust application-layer transport that carries one
opaque TCP stream over a mutually authenticated Noise IK channel. The transport
is application-agnostic: it does not parse HTTP, SSE, WebSockets, OAuth, SOCKS,
proxy protocols, or application payloads.
...
may be an application service, a forward proxy, or another operator-controlled
TCP service.
...
The client process keeps only its loopback listener open. It does not maintain
a background tunnel, poll the remote host, or retry while idle. If an
application uses the local listener while the configured ingress is
unreachable, that request fails after the configured connection timeout; the
client process continues running and works again when the ingress becomes
reachable.
...
3. Run the server on the ingress host and the client on the application host.
   Use `doctor --config ...` on the client to verify the real pinned Noise peer
   before redirecting an application to the local listener.
...
- The system intentionally does not conceal metadata such as timing, packet
  sizes, ingress IP, or the existence of traffic.
